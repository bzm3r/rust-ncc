...
OUT_DIR = "../../output/py_comp/{}/"
OUT_PATH_TEMPLATE = hio.FILE_NAME_TEMPLATE


def read_save_file(model_ty, final_t, num_int_steps, num_cells, cil_mag,
                   coa_mag):
    od = os.path.normpath(OUT_DIR.format(model_ty))
    fn = os.path.normpath(OUT_PATH_TEMPLATE.format(final_t=final_t,
...
def gen_data_dict_per_cell_int_steps(data):
    ...
    for cell_ix in range(num_cells):
        for label in hio.DATA_LABELS:
            ...
            for cells_in_tstep in tstep_data:
                int_steps = cells_in_tstep["cells"][cell_ix]["int_steps"]
                for i in range(num_int_steps):
                    data_per_int_step_for_cell.append(
                        copy.deepcopy(int_steps[i][label])
                    )
            data_per_int_step_for_cell = np.array(data_per_int_step_for_cell)
            data_dict_per_cell[cell_ix][label] = copy.deepcopy(
