...
fn model(app: &App) -> Model {
    ...
    let notes = bars::WithStarts::new(bars.iter().cloned(), PPQN)
        ...
        .map(|(i, (time_sig, start))| {
            ...
            let pitch = pitch::Step((24 + (i * 5) % 12) as f32)
                .to_letter_octave();
    ...
    let octave_envelope = {
        ...
        let points =
            bars::WithStarts::new(bars.iter().cloned(), PPQN)
                .enumerate()
                .flat_map(|(i, (ts, mut start))| {
                    ...
                    std::iter::from_fn(move || {
                        ...
                        let end =
                            if end > bar_end { bar_end } else { end };
                        let point = Point {
                            ticks: end,
                            value: 1
                                + ((i as i32 + j as i32) * 3) % 12,
                        };
                        start = end;
                        j += 1;
                        Some(point)
...
fn update(_app: &App, model: &mut Model, update: Update) {
    ...
    let tempo_value =
        timeline_data.tempo_envelope.y(timeline_data.playhead_ticks);
    ...
    let delta_ticks = time::Ms(delta_secs * ONE_SECOND_MS)
        .to_ticks(current_bpm, PPQN);
    let total_duration_ticks = timeline::bars_duration_ticks(
        timeline_data.bars.iter().cloned(),
        PPQN,
    );
    let previous_playhead_ticks =
        timeline_data.playhead_ticks.clone();
    timeline_data.playhead_ticks = (timeline_data.playhead_ticks
        + delta_ticks)
...
fn set_widgets(ui: &mut UiCell, ids: &Ids, data: &mut TimelineData) {
    ...
    {
        let ruler =
            track::Ruler::new(context.ruler, &context.bars, PPQN)
                .color(color);
...

    {
        // Piano roll.
        let piano_roll =
            track::PianoRoll::new(&context.bars, PPQN, &notes[..])
                .color(color);
        ...
        macro_rules! numeric_automation {
            ($envelope:expr) => {
                let track = {
                    let automation = track::automation::Numeric::new(
                        &context.bars,
                        PPQN,
                        $envelope,
                    )
                ...
                for event in track.event {
                    ...
                    match event {
                        Event::Interpolate(number) => {
                            println!("{}", number)
                        }
                        Event::Mutate(mutate) => {
                            mutate.apply($envelope)
                        }
                    }
                }
            };
        ...
        // Toggle automation.
        let track = {
            let automation = track::automation::Toggle::new(
                &context.bars,
                PPQN,
                toggle_envelope,
            )
        ...
        // Bang automation.
        let track = {
            let automation = track::automation::Bang::new(
                &context.bars,
                PPQN,
                bang_envelope,
            )
...
fn key_pressed(_app: &App, model: &mut Model, key: Key) {
    match key {
        ...
        Key::R => {
            ...
            model.timeline_data.notes =
                bars::WithStarts::new(bars.iter().cloned(), PPQN)
                    .enumerate()
                    .map(|(i, (time_sig, start))| {
                        let end =
                            start + time_sig.ticks_per_bar(PPQN);
                        let period = timeline::Period { start, end };
                        let pitch = pitch::Step(
                            (24 + (i * (random::<usize>() % 11)) % 12)
                                as f32,
                        )
                        .to_letter_octave();
                        piano_roll::Note { period, pitch }
        ...
        Key::S => {
            ...
            #[cfg(feature = "serde1")]
            {
                nannou::io::save_to_json(
                    "./saved_timeline_data.json",
                    &model.timeline_data,
                )
        ...
        Key::L => {
            ...
            #[cfg(feature = "serde1")]
            {
                if let Ok(new_data) = nannou::io::load_from_json(
                    "./saved_timeline_data.json",
                ) {
