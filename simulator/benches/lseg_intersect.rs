...
use rand_pcg::Pcg32;
use simulator::math::geometry::{IntersectCalcResult, LineSeg2D};
use simulator::math::v2d::V2d;
...
fn check_equivalence(lsegs: &[LineSeg2D]) {
    for (i, lseg_a) in lsegs.iter().enumerate() {
        'inner: for (_, lseg_b) in lsegs[(i + 1)..].iter().enumerate() {
            ...
            match (lseg_a.check_intersection(lseg_b), calc_result) {
                ...
                (true, _) => {
                    println!("lseg_a: {}, lseg_b: {}", lseg_a, lseg_b);
                    println!(
                        "check_lseg_intersect returns: {}",
                        lseg_a.check_intersection(lseg_b)
                    );
                    println!("calc_lseg_intersect returns: {}", calc_result);
...
fn check_only(lsegs: &[LineSeg2D]) {
    for (i, lseg_a) in lsegs.iter().enumerate() {
        for lseg_b in lsegs[(i + 1)..].iter() {
            let _ = lseg_a.check_intersection(lseg_b);
        }
    }
}
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("calc", |b| b.iter(|| calc(black_box(&lsegs))));
    group.finish();
}
