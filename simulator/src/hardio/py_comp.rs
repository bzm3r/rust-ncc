...
#[derive(
    Clone, Deserialize, Serialize, Default, Debug, PartialEq,
)]
pub struct Header {
...
impl StepData {
    pub fn save(&mut self, max_cells: usize, cell_data: CellData) {
        if self.cells.len() >= max_cells {
            panic!("cannot save cell, already have {} cells stored (max: {})",
                   self.cells.len(),
                   max_cells
            )
        } else {
...
impl Writer {
    ...
    pub fn begin_cell_save(&mut self) {
        match (
            ...
            (_, _) => {
                panic!("cannot begin cell save: curr_cell = {} (max: {}), \
                curr_int_step = {} (max: {})", 
                       self.curr_cell_ix, self.num_cells, self.curr_int_step_ix, self.num_int_steps)
            }
        }
    }
    ...
    pub fn finish_cell_save(&mut self) {
        if self.curr_int_step_ix == self.num_int_steps {
            ...
            if self.can_save_cell {
                self.curr_step_data.save(
                    self.num_cells,
                    self.curr_cell_data.clone(),
                );
                self.curr_cell_ix += 1;
            } else {
    ...
    pub fn finish_tstep_save(&mut self) {
        if self.curr_cell_ix == self.num_cells {
            ...
            panic!("cannot finish tstep save; only have {} cells (required: {})",
                   self.curr_cell_ix, self.num_cells)
        }
    }
    ...
    pub fn finish(&mut self) {
        ...
        let file_path: PathBuf =
            [self.out_dir.clone(), self.file_name.clone()]
                .iter()
                .collect();
