...
use crate::exp_setup::{CellGroup, Experiment, ExperimentType, GroupBBox};
...
fn group_bbox(
    ...
    let inter_group_sep =
        char_quants.normalize(&raw_params.cell_diam.scale(0.5 * 0.02));
    let bottom_left = V2d {
        x: char_quants.normalize(&raw_params.cell_diam.scale(group_ix as f64))
...
fn raw_params(group_ix: usize, randomization: bool) -> RawParameters {
    ...
    let rac_distrib = distrib_gens::specific_uniform(0.3, specific_rac);
    let init_rac = RgtpDistribution::new(rac_distrib, rac_distrib);

    let rho_distrib = distrib_gens::specific_uniform(0.3, specific_rho);
...
fn make_cell_group(
    ...
    CellGroup {
        num_cells,
        layout: group_bbox(group_ix, char_quants, &raw_params).unwrap(),
        parameters,
    }
}
...
pub fn generate(dirs: Directories, args: ExperimentArgs) -> Vec<Experiment> {
    ...
    let (num_cells, py_main, run_python) = if let ExperimentType::PyCompare {
        num_cells,
        py_main,
        run_python,
    } = &ty
    ...
    seeds
        ...
        .map(|&seed| {
            ...
            let cgs = make_cell_groups(&char_quants, randomization, num_cells);
