...
use crate::cell::chemistry::{gen_rgtp_distrib, RacRandomState, RgtpLayout};
...
impl Cell {
    ...
    pub fn simulate_euler(
        ...
        for i in 0..nsteps {
            ...
            println!("{}", state);
            let dep_vars =
                State::calc_dep_vars(&state, &self.rac_randomization, inter_state, parameters);
            println!("{}", dep_vars);
            println!("++++++++++++");
            let delta =
                State::dynamics_f(&state, &self.rac_randomization, &inter_state, parameters);
