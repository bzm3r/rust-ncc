use crate::cell::chemistry::{
    calc_conc_rgtps, calc_kdgtps_rac,
    calc_kdgtps_rho, calc_kgtps_rac,
    calc_kgtps_rho, calc_net_fluxes,
    calc_rgtp_state, RacRandState,
    RgtpDistribution,
};
use crate::cell::mechanics::{
    calc_cyto_forces, calc_edge_forces,
    calc_edge_vecs, calc_rgtp_forces,
};
use crate::interactions::{
    CellInteractions, RgtpState,
};
use crate::math::v2d::V2d;
use crate::math::{
    hill_function3, max_f32, min_f32,
};
...
impl Add for CoreState {
    ...
    fn add(
        ...
        for i in 0..(NVERTS) {
            vertex_coords[i] = self
                .vertex_coords[i]
                + rhs.vertex_coords[i];
            rac_acts[i] = self.rac_acts
                [i]
                + rhs.rac_acts[i];
            rac_inacts[i] = self
                .rac_inacts[i]
                + rhs.rac_inacts[i];
            rho_acts[i] = self.rho_acts
                [i]
                + rhs.rho_acts[i];
            rho_inacts[i] = self
                .rho_inacts[i]
...
pub struct ChemState {
    ...
    pub rac_act_net_fluxes:
        [f32; NVERTS],
    pub rac_inact_net_fluxes:
        [f32; NVERTS],
    pub kdgtps_rho: [f32; NVERTS],
    pub kgtps_rho: [f32; NVERTS],
    pub rac_cyto: f32,
    pub rho_cyto: f32,
    pub rho_act_net_fluxes:
        [f32; NVERTS],
    pub rho_inact_net_fluxes:
        [f32; NVERTS],
    pub x_tens: f32,
}
...
impl CoreState {
    ...
    pub fn calc_geom_state(
        &self,
    ) -> GeomState {
        let evs = calc_edge_vecs(
            &self.vertex_coords,
        );
        let mut edge_lens =
            [0.0_f32; NVERTS];
    ...
    pub fn calc_mech_state(
        ...
        let mut edge_strains =
            [0.0_f32; NVERTS];
        (0..NVERTS).for_each(|i| {
            edge_strains[i] = (edge_lens
                [i]
                / parameters
                    .rest_edge_len)
                - 1.0
    ...
    pub fn calc_chem_state(
        ...
        let mut avg_edge_lens: [f32;
            NVERTS] = [0.0_f32; NVERTS];
        (0..NVERTS).for_each(|i| {
            let im1 = circ_ix_minus(
                i as usize, NVERTS,
            );
    ...
    pub fn dynamics_f(
        ...
        let geom_state =
            Self::calc_geom_state(
                state,
            );
        let mech_state =
            Self::calc_mech_state(
                state,
                &geom_state,
                parameters,
            );
        let chem_state =
            Self::calc_chem_state(
                state,
                &geom_state,
                &mech_state,
                rac_rand_state,
        ...
        for i in 0..NVERTS {
            let inactivated_rac =
                chem_state.kdgtps_rac
                    [i]
                    * state.rac_acts[i];
            let activated_rac =
                chem_state.kgtps_rac[i]
                    * state.rac_inacts
                        [i];
            let delta_rac_activated =
                activated_rac
                    - inactivated_rac;
            ...
            let inactivated_rho =
                chem_state.kdgtps_rho
                    [i]
                    * state.rho_acts[i];
            let activated_rho =
                chem_state.kgtps_rho[i]
                    * state.rho_inacts
                        [i];
            let delta_rho_activated =
                activated_rho
                    - inactivated_rho;
    ...
    pub fn sum(&self) -> f32 {
        ...
        for i in 0..(NVERTS) {
            r += self.vertex_coords[i]
                .x
                + self.vertex_coords[i]
                    .y;
            r += self.rac_acts[i];
            r += self.rac_inacts[i];
            r += self.rho_acts[i];
