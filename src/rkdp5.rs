...
use crate::cell::{CellState, InteractionState};
use crate::parameters::Parameters;

type CellDynamicsFn = fn(dt: f32, state: &CellState, interaction_state: &InteractionState, parameters: &Parameters) -> CellState;
...
const A4: [f32; 4] = [19372.0/6561.0, -25360.0/2187.0, 64448.0/6561.0, -212.0/729.0];
const A5: [f32; 5] = [9017.0/3168.0, -355.0/33.0, 46732.0/5247.0, 49.0/176.0, -5103.0/18656.0];
const A6: [f32; 6] = [35.0/384.0, 0.0, 500.0/1113.0, 125.0/192.0, -2187.0/6784.0, 11.0/84.0];
...
impl Ks {
    fn calc(premul_ajs: &PreMulA, f: &CellDynamicsFn, dt_primes: [f32; 6], init_state: &CellState, inter_state: &InteractionState, params: &Parameters) -> Ks {
        let PreMulA {
            a1, a2, a3, a4, a5, a6
        } = premul_ajs;

        let k0 = f(dt_primes[0], init_state, inter_state, params);

        let k1 = {
            let kp = init_state + a1 * &k0;
            f(dt_primes[1], &kp, inter_state, params)
        };
        ...
        let k5 = {
            let kp = init_state + a5[0] * &k0 + a5[1] * &k1 + a5[2] * &k2 + a5[3] * &k3 + a5[4] * &k4;
            f(dt_primes[1], &kp, inter_state, params)
        };

        let k6 = {
            let kp = init_state + a6[0] * &k0 + a6[1] * &k1 + a6[2] * &k2 + a6[3] * &k3 + a6[4] * &k4 + a6[5] * &k5;
            f(dt_primes[1], &kp, inter_state, params)
        };
...
pub fn rkdp5(mut dt: f32, f: fn(f32, &CellState, &InteractionState, &Parameters) -> CellState, mut init_state: CellState, inter_state: &InteractionState, parameters: &Parameters, mut aux_args: AuxArgs) -> Solution {
    let AuxArgs {
        max_iters, atol, rtol, init_h_factor
    } = aux_args;
    ...
    while num_iters < max_iters {
        ...
        let dt_primes = {
            let mut dt_primes = [0.0_f32; 6];
            (0..6).for_each(|j| dt_primes[j] = h*C[j]);
            dt_primes
        };

        let Ks {
            k0, k1, k2, k3, k4, k5, k6
        } = Ks::calc(&premul_as, &f, dt_primes, &init_state, inter_state, parameters);
        ...
        let y1 = init_state + a6[0] * k0 + a6[1] * k1 + a6[2] * k2
            + a6[3] * k3 + a6[4] * k4 + a6[5] * k5;
        ...
        let y1_hat = init_state + h * (B_HAT[0] * k0 + B_HAT[1] * k1 + B_HAT[2] * k2
                + B_HAT[3] * k3 + B_HAT[4] * k4 + B_HAT[5] * k5 + B_HAT[6] * k6);

        // Equations 4.10, 4.11, Hairer,Wanner&Norsett Solving ODEs Vol. 1
        let sc = init_state.abs().max(&y1.abs()).scalar_mul(rtol).scalar_add(atol);
        let delta = y1 - y1_hat;
