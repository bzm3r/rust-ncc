...
use crate::experiments::{
    CellGroup, Experiment,
};
use crate::interactions::{
    CellInteractions,
    InteractionGenerator, RgtpState,
};
use crate::math::v2d::V2d;
use crate::parameters::{
    Parameters, WorldParameters,
};
use crate::world::hardio::{
    save_data, save_schema,
};
use crate::NVERTS;
use avro_rs::Writer;
...
impl World {
    pub fn new(
        ...
        for (ix, gix) in cell_group_ixs
            .into_iter()
            .enumerate()
        {
            let parameters =
                &group_parameters[gix];
            let normal =
                Lazy::new(|| {
                    Normal::new(
                        parameters
    ...
    pub fn simulate(
        ...
        let num_tsteps = (final_tpoint
            / self.tstep_length)
            .ceil()
            as u32;
        while self.cell_states.tstep
            < num_tsteps
        {
            println!("========================================");
            println!(
                "tstep: {}/{}",
                self.cell_states.tstep,
                num_tsteps
            );
    ...
    pub fn save_history(
        ...
        let name = "history";
        let schema =
            Cells::schematize(None);
        save_schema(
            name, &schema, output_dir,
        );
        let mut writer = Writer::new(
            &schema,
            Vec::new(),
        ); //Writer::with_codec(&self.state_schema, Vec::new(), avro_rs::Codec::Deflate);
...
fn gen_cell_centroids(
        ...
        >= *num_cells
    {
        ...
        let first_cell_centroid = V2d {
            x: layout.bottom_left.x
                + cell_r,
            y: layout.bottom_left.y
                + cell_r,
        };
        let delta_x = V2d {
            x: 2.0 * cell_r,
