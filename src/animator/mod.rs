...
fn draw_cell_poly(
    context: &Context,
    cell_poly: &[V2d; NVERTS],
) {
    context
        .set_source_rgb(0.0, 0.0, 0.0);
    context.move_to(
        cell_poly[0].x as f64,
        cell_poly[0].y as f64,
    );
    cell_poly[1..].iter().for_each(
        |vc| {
            context.line_to(
                vc.x as f64,
                vc.y as f64,
...
impl DrawingData {
    ...
    pub fn get_cell_polys(
        &self,
        frame: usize,
    ) -> &[[V2d; NVERTS]] {
        let start =
            frame * self.num_cells;
        let end =
            start + self.num_cells;
...
pub fn create_animation(
    ...
    let data =
        DrawingData::from_history(
            history, 1280, 720, 2.0,
        );
    create_mp4(
        &data,
        1000,
